...
import {
  ...
  Brain, CheckCircle2, XCircle, Sparkles,
  DollarSign, Trophy
} from "lucide-react";
...
export default function EgxPage() {
  ...
  const fetchPicks = useCallback(async (force = false) => {
    ...
    try {
      const res = await fetch(`/api/signals/picks${force ? "?force=1" : ""}`);
  ...
  useEffect(() => {
    if (tab === "picks" && !picksData) fetchPicks();
  }, [tab, picksData, fetchPicks]);
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/signals/results");
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "24px", maxWidth: "1280px", margin: "0 auto" }}>
      ...
      {/* ── Tab switcher ── */}
      <div style={{ display: "flex", gap: 4, marginBottom: 16, background: "rgba(255,255,255,0.03)", padding: 4, borderRadius: 10, width: "fit-content", flexWrap: "wrap" }}>
        ...
        <button style={{
          ...TAB_STYLE(tab === "picks"),
          background: tab === "picks" ? "rgba(34,197,94,0.15)" : "transparent",
          color: tab === "picks" ? "#22C55E" : "var(--text-muted)",
          outline: tab === "picks" ? "1px solid rgba(34,197,94,0.3)" : "none",
        }} onClick={() => setTab("picks")}>
          <Sparkles size={12} style={{ display: "inline", marginRight: 5 }} />
          🟢 توصيات الشراء
        </button>
        <button style={{
          ...TAB_STYLE(tab === "results"),
          background: tab === "results" ? "rgba(99,102,241,0.15)" : "transparent",
          color: tab === "results" ? "#818cf8" : "var(--text-muted)",
          outline: tab === "results" ? "1px solid rgba(99,102,241,0.3)" : "none",
      ...
      {tab === "picks" && (
        ...
        {picksData?.goldPick && (
          <div className="glass-card" style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                <p style={{ fontSize: 16, fontWeight: 800, color: "#EAB308" }}>الذهب (XAU/EGP)</p>
                <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 3 }}>{picksData.goldPick.reason}</p>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
              <div style={{ textAlign: "center" }}>
                <p style={{ fontSize: 10, color: "var(--text-muted)" }}>السعر (أوقية EGP)</p>
                <p style={{ fontSize: 18, fontWeight: 800, color: "var(--text-primary)" }}>{picksData.goldPick.price.toLocaleString()}</p>
              </div>
              <div style={{ textAlign: "center" }}>
                <p style={{ fontSize: 10, color: "var(--text-muted)" }}>التغير اليوم</p>
                <p style={{ fontSize: 16, fontWeight: 700, color: picksData.goldPick.change >= 0 ? "#22C55E" : "#EF4444" }}>
                  {picksData.goldPick.change >= 0 ? "+" : ""}{picksData.goldPick.change.toFixed(2)}%
                </p>
              </div>
              <span style={{
                fontSize: 11, fontWeight: 800, padding: "5px 14px", borderRadius: 20,
                background: picksData.goldPick.signal === "STRONG_BUY" ? "rgba(34,197,94,0.15)" : "rgba(234,179,8,0.15)",
                color: picksData.goldPick.signal === "STRONG_BUY" ? "#22C55E" : "#EAB308",
                border: `1px solid ${picksData.goldPick.signal === "STRONG_BUY" ? "rgba(34,197,94,0.3)" : "rgba(234,179,8,0.3)"}`,
              }}>
                {picksData.goldPick.signal.replace("_", " ")} · {picksData.goldPick.score}/100
              </span>
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 800, color: "var(--text-primary)" }}>🟢 أفضل أسهم EGX للشراء اليوم</h2>
            <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 3 }}>
              مرتبة حسب قوة الإشارة · تعتمد على TradingView + RSI + MACD
              {picksData?.stats?.generatedAt && ` · آخر تحديث: ${new Date(picksData.stats.generatedAt).toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit" })}`}
            </p>
          </div>
          <button className="btn-ghost" onClick={() => { setPicksData(null); fetchPicks(true); }}
            style={{ padding: "7px 14px", display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
        ...
        {picksLoading ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 12 }}>
            {[...Array(6)].map((_, i) => <div key={i} className="skeleton" style={{ height: 180, borderRadius: 14 }} />)}
          </div>
        ) : !picksData?.picks?.length ? (
          <div className="glass-card" style={{ padding: "60px 24px", textAlign: "center" }}>
            <Sparkles size={36} style={{ margin: "0 auto 16px", opacity: 0.2 }} />
            <p style={{ fontSize: 15, fontWeight: 600, color: "var(--text-primary)" }}>جاري تحليل السوق...</p>
            <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 8 }}>اضغط تحديث لمسح السوق الآن</p>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 12 }}>
            {picksData.picks.map((pick: any, idx: number) => {
              const isStrong = pick.signal === "STRONG_BUY";
              const sc = isStrong ? "#22C55E" : "#818cf8";
              const rankColor = idx === 0 ? "#F59E0B" : idx === 1 ? "#94A3B8" : idx === 2 ? "#CD7F32" : "var(--text-muted)";
              return (
                <div key={pick.symbol} style={{
                  padding: "18px 20px", borderRadius: 14,
                  background: isStrong ? "rgba(34,197,94,0.04)" : "rgba(255,255,255,0.02)",
                  border: `1px solid ${isStrong ? "rgba(34,197,94,0.2)" : "rgba(99,102,241,0.15)"}`,
                  position: "relative",
                }}>
                  {/* Rank */}
                  <span style={{ position: "absolute", top: 12, right: 14, fontSize: 12, fontWeight: 800, color: rankColor }}>#{idx + 1}</span>

                  {/* Header */}
                  <div style={{ marginBottom: 12, paddingRight: 30 }}>
                    <p style={{ fontSize: 18, fontWeight: 900, color: "var(--text-primary)" }}>{pick.symbol}</p>
                    <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{pick.name}</p>
                  </div>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ fontSize: 10, color: "var(--text-muted)" }}>قوة الإشارة</span>
                      <span style={{ fontSize: 12, fontWeight: 800, color: sc }}>{pick.score}/100</span>
                    </div>
                    <div style={{ height: 5, borderRadius: 99, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${pick.score}%`, borderRadius: 99, background: isStrong ? "linear-gradient(90deg,#16a34a,#22C55E)" : "linear-gradient(90deg,#4f46e5,#818cf8)", transition: "width 0.8s" }} />
                    </div>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                    {[
                      { label: "السعر", value: `${pick.price.toFixed(2)} EGP` },
                      { label: "التغير", value: `${pick.change >= 0 ? "+" : ""}${pick.change.toFixed(2)}%`, color: pick.change >= 0 ? "#22C55E" : "#EF4444" },
                      { label: "RSI", value: pick.rsi !== null ? pick.rsi.toFixed(0) : "—", color: pick.rsi !== null && pick.rsi < 35 ? "#22C55E" : "var(--text-primary)" },
                    ].map(({ label, value, color }) => (
                      <div key={label} style={{ padding: "8px", background: "rgba(255,255,255,0.03)", borderRadius: 8, textAlign: "center" }}>
                        <p style={{ fontSize: 9, color: "var(--text-muted)", marginBottom: 3 }}>{label}</p>
                  ...
                  {/* Reason */}
                  <p style={{ fontSize: 11, color: "var(--text-muted)", background: "rgba(255,255,255,0.03)", padding: "8px 10px", borderRadius: 8, lineHeight: 1.5 }}>
                    💡 {pick.reason}
                  </p>
                </div>
              );
      ...
      {tab === "results" && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 20 }}>
            {[
              { label: "دقة التوقعات", value: resultsData?.overall?.accuracy !== null ? `${resultsData?.overall?.accuracy}%` : "—", color: (resultsData?.overall?.accuracy ?? 0) >= 55 ? "#22C55E" : "#EAB308", sub: "Direction Accuracy" },
              { label: "إجمالي التوقعات", value: resultsData?.overall?.totalPredictions ?? 0, color: "var(--text-primary)", sub: "كل الأيام" },
              { label: "صحيح", value: resultsData?.overall?.correct ?? 0, color: "#22C55E", sub: "اتجاه صحيح" },
              { label: "متوسط العائد", value: resultsData?.overall?.avgGainPct !== null ? `${resultsData?.overall?.avgGainPct}%` : "—", color: (resultsData?.overall?.avgGainPct ?? 0) >= 0 ? "#22C55E" : "#EF4444", sub: "لو اشتريت كل توصية" },
              ...
              <div key={label} className="glass-card" style={{ padding: "18px", textAlign: "center" }}>
                ...
                {resultsLoading ? <div className="skeleton" style={{ width: 60, height: 32, margin: "0 auto" }} /> :
                  <p style={{ fontSize: 28, fontWeight: 800, color }}>{value}</p>}
                <p style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 4 }}>{sub}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
            <div>
              <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)" }}>📋 سجل التوقعات اليومي</h2>
              <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>هل توقعات الأمس كانت صح أم غلط؟ · يتم التحقق تلقائياً يومياً</p>
            </div>
            <button className="btn-ghost" onClick={() => { setResultsData(null); fetchResults(); }}
              style={{ padding: "7px 14px", display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
              <RefreshCw size={13} /> تحديث
            </button>
          </div>
          ...
          {resultsLoading ? (
            [...Array(3)].map((_, i) => <div key={i} className="skeleton" style={{ height: 120, borderRadius: 14, marginBottom: 12 }} />)
          ) : !resultsData?.snapshots?.length ? (
            <div className="glass-card" style={{ padding: "60px 24px", textAlign: "center" }}>
              <CheckCircle2 size={40} style={{ margin: "0 auto 16px", opacity: 0.15 }} />
              <p style={{ fontSize: 15, fontWeight: 600, color: "var(--text-primary)" }}>لا توجد نتائج بعد</p>
              <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 8, lineHeight: 1.6 }}>
                النظام يحفظ توصيات كل يوم تلقائياً الساعة 10:05 صباحاً<br />
                ويتحقق من النتائج في اليوم التالي · ارجع غداً لترى أول نتيجة!
              </p>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {resultsData.snapshots.map((snap: any) => (
                <div key={snap.date} className="glass-card" style={{ padding: "18px 20px" }}>
                  {/* Day header */}
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 13, fontWeight: 800, color: "var(--text-primary)" }}>
                        {new Date(snap.date).toLocaleDateString("ar-EG", { weekday: "long", day: "numeric", month: "long" })}
                      </span>
                    </div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {snap.stats.accuracy !== null && (
                        <span style={{
                          fontSize: 12, fontWeight: 800, padding: "3px 12px", borderRadius: 20,
                          background: snap.stats.accuracy >= 55 ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.12)",
                          color: snap.stats.accuracy >= 55 ? "#22C55E" : "#EF4444",
                          border: `1px solid ${snap.stats.accuracy >= 55 ? "rgba(34,197,94,0.3)" : "rgba(239,68,68,0.25)"}`,
                        }}>
                          دقة اليوم: {snap.stats.accuracy}%
                        </span>
                      )}
                      <span style={{ fontSize: 11, color: "var(--text-muted)", padding: "3px 8px" }}>
                        ✅ {snap.stats.correct} · ❌ {snap.stats.incorrect} · ⏳ {snap.stats.pending}
                      </span>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {snap.picks.map((pick: any) => {
                      const isPending = pick.correct === null;
                      const isWin = pick.correct === true;
                      const bc = isPending ? "rgba(99,102,241,0.15)" : isWin ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)";
                      const bg = isPending ? "rgba(99,102,241,0.03)" : isWin ? "rgba(34,197,94,0.03)" : "rgba(239,68,68,0.03)";
                      return (
                        <div key={pick.id} style={{
                          display: "flex", justifyContent: "space-between", alignItems: "center",
                          padding: "10px 14px", borderRadius: 10, background: bg, border: `1px solid ${bc}`,
                          flexWrap: "wrap", gap: 8,
                        }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <span style={{ fontSize: 16 }}>{isPending ? "⏳" : isWin ? "✅" : "❌"}</span>
                            <div>
                              <span style={{ fontSize: 14, fontWeight: 800, color: "var(--text-primary)" }}>
                                {pick.type === "GOLD" ? "🥇 الذهب" : pick.symbol}
                              </span>
                              <p style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 1 }}>{pick.reason}</p>
                            </div>
                          </div>
                          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                            <div style={{ textAlign: "center" }}>
                              <p style={{ fontSize: 9, color: "var(--text-muted)" }}>دخول</p>
                              <p style={{ fontSize: 12, fontWeight: 700, color: "var(--text-primary)" }}>{pick.entryPrice.toFixed(2)}</p>
                            </div>
                            {pick.exitPrice && (
                              <div style={{ textAlign: "center" }}>
                                <p style={{ fontSize: 9, color: "var(--text-muted)" }}>خروج</p>
                                <p style={{ fontSize: 12, fontWeight: 700, color: "var(--text-primary)" }}>{pick.exitPrice.toFixed(2)}</p>
                              </div>
                            )}
                            {pick.change !== null && (
                              <span style={{ fontSize: 14, fontWeight: 800, color: pick.change >= 0 ? "#22C55E" : "#EF4444" }}>
